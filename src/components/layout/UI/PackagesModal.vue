...
const props = defineProps(['isVisible', 'logoType', 'logoHeading', 'logoCategory']);
...
const closeModalOnOverlayClick = (event) => {
  ...
  const submitForm = async () => {
    try {
      loading.value = true;
      formData.value.logoType = props.logoType;
      formData.value.logoHeading = props.logoHeading;
      formData.value.logoCategory = props.logoCategory;
      // Make a POST request to your backend API with the full URL
      const response = await axios.post(`${baseUrl}/packages`, formData.value);
